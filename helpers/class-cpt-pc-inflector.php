...
class CPT_PC_Inflector {
	 ...
	 * @var array
	 */
	protected static $_initial_state = array();
	...
	public static function reset() {
		if ( empty( static::$_initial_state ) ) {
			static::$_initial_state = get_class_vars( __CLASS__ );

			return;
		}
		foreach ( static::$_initial_state as $key => $val ) {
	...
	public static function camelize( $string, $delimiter = '_' ) {
		$cache_key = __FUNCTION__ . $delimiter;

		$result = static::_cache( $cache_key, $string );

		if ( $result === false ) {
			$result = str_replace( ' ', '', static::humanize( $string, $delimiter ) );
			static::_cache( $cache_key, $string, $result );
		}
	...
	/**
	 * Returns corresponding table name for given model $class_name. ("people" for the model class "Person").
	 *
	...
	public static function tableize( $class_name ) {
		$result = static::_cache( __FUNCTION__, $class_name );

		if ( $result === false ) {
			$result = static::pluralize( static::underscore( $class_name ) );
			static::_cache( __FUNCTION__, $class_name, $result );
		}
	...
	public static function classify( $table_name ) {
		$result = static::_cache( __FUNCTION__, $table_name );

		if ( $result === false ) {
			$result = static::camelize( static::singularize( $table_name ) );
			static::_cache( __FUNCTION__, $table_name, $result );
		}
